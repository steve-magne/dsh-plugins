...
assert.equal(registeredEntries[0].options.id, "merge-pilot");
assert.equal(
...
assert.match(
	...
	/\.mp-panel\{/,
	"stylesheet must style the pilot panel",
);
assert.match(
	sandbox.appendedStyle.textContent,
	/\.mp-dot-fixing\{/,
	"stylesheet must color pilot statuses",
);

console.log("client-bundle contract OK:");
console.log("  - registers under id @dsh-plugins/merge-pilot");
console.log("  - injects into slot 'shell.overlay' as entry 'merge-pilot'");
