...
assert.equal(registeredEntries[0].options.id, "agent-terminal");
assert.equal(
...
assert.match(
	...
	/\.at-panel\{/,
	"stylesheet must style the takeover console panel",
);
assert.ok(
	source.includes('"@dsh-plugins/agent-terminal/console.css"'),
	"stylesheet tag must carry the idempotent data-plugin-css id",
);

console.log("client-bundle contract OK:");
console.log("  - registers under id @dsh-plugins/agent-terminal");
console.log("  - injects into slot 'shell.overlay' as entry 'agent-terminal'");
