...
assert.equal(registeredEntries[0].options.id, "changes-lens");
assert.equal(
...
assert.match(
	...
	/\.cl-panel\{/,
	"stylesheet must style the lens panel",
);
assert.match(
	sandbox.appendedStyle.textContent,
	/\.cl-line-add\{/,
	"stylesheet must color diff additions",
);

console.log("client-bundle contract OK:");
console.log("  - registers under id @dsh-plugins/changes-lens");
console.log("  - injects into slot 'shell.overlay' as entry 'changes-lens'");
