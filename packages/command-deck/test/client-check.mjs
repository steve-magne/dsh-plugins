...
assert.equal(registeredEntries[0].options.id, "command-deck");
assert.equal(
...
assert.match(
	...
	/\.cd-panel\{/,
	"stylesheet must style the deck panel",
);

console.log("client-bundle contract OK:");
console.log("  - registers under id @dsh-plugins/command-deck");
console.log("  - injects into slot 'shell.overlay' as entry 'command-deck'");
