...
assert.equal(registeredEntries[0].options.id, "issue-starter");
assert.equal(
...
assert.match(
	...
	/\.is-panel\{/,
	"stylesheet must style the issue-starter panel",
);
assert.ok(source.includes('"/issues/preview"'), "calls the preview endpoint");
assert.ok(source.includes('"/issues/start"'), "calls the launch endpoint");

console.log("client-bundle contract OK:");
console.log("  - registers under id @dsh-plugins/issue-starter");
console.log("  - injects into slot 'shell.overlay' as entry 'issue-starter'");
